...
function CountUnit({ value, label }) {
  return (
    <div className="flex flex-col items-center gap-2.5">
      <div className="flex items-center justify-center bg-white border border-gray-200 rounded-2xl shadow-sm"
        style={{ width: 88, height: 96 }}>
        <span className="text-5xl font-bold text-gray-700 tabular-nums select-none">{pad(value)}</span>
      </div>
      <span className="text-[10px] font-black tracking-[0.2em] uppercase text-gray-400">{label}</span>
    </div>
  );
}
  ...
  return (
    <div className="-ml-1 w-[97%] h-[96%] bg-[#ECECEC] rounded-xl flex flex-col p-3 gap-2.5">
      ...
      <div className="flex items-center justify-between flex-shrink-0">
        ...
        <button onClick={fetchNext} disabled={fetching}
        ...
        {fetching ? (
          ...
          <div className="flex flex-col items-center gap-8 relative">
            <div className="flex flex-col items-center gap-2">
              ...
              <p className="text-2xl font-bold text-gray-800 mt-1">{fmtDate(schedule.flag_ceremony_date)}</p>
